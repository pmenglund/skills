...
5) Reuse existing Playwright setup when present; otherwise copy boilerplate into `./e2e/playwright/`.
...
- Do not copy Playwright boilerplate into the repo root.
- Copy `assets/playwright/` into `./e2e/playwright/` by default.
- If the repository already has Playwright, merge tests/config without replacing existing root config files.
- Never overwrite existing `package.json` or `playwright.config.*`; update scripts/config entries surgically.
- Update `E2E_BASE_URL` and `data-testid` selectors to match the UI.

## Verification steps (required)
- `go test ./...`
- install Playwright dependencies if needed:
  - run package manager install in the Playwright directory
  - run `npx playwright install chromium`
- run Playwright tests (headless), for example `npm run test:e2e`
